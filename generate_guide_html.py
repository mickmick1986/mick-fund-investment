...
// 实时快照优先从主分支直接读取，避免每分钟提交触发 Pages 整站部署；同源文件只作为网络故障回退。
...
function loadLiveSnapshot() {{
  ...
  function loadAt(index) {{
    ...
    var separator = LIVE_SNAPSHOT_URLS[index].indexOf('?') >= 0 ? '&' : '?';
    return fetch(LIVE_SNAPSHOT_URLS[index] + separator + cacheBuster, {{ cache: 'no-store' }})
      ...
      .then(function(snapshot) {{
        ...
        if (!snapshot || snapshot.trade_date !== today || !snapshot.estimates) throw new Error('snapshot stale');
        return snapshot;
      }})
      .catch(function() {{ return loadAt(index + 1); }});
  }}
