...
// 时段：9:30-11:30/13:00-14:29 每10分钟 | 14:30-15:00 每1分钟 | 盘后60秒整页刷新
var LIVE_INTERVAL = 60 * 1000;      // 1分钟（14:30-15:00高频窗口）
var SLOW_INTERVAL = 10 * 60 * 1000; // 10分钟（盘中慢速）
...
// jsDelivr 镜像支持跨域读取，避免浏览器直连 GitHub API/Raw 在部分网络下失败。
// 原始 GitHub 地址和同源文件只作故障回退，不承担盘中主读取职责。
var LIVE_SNAPSHOT_URLS = [
  'https://cdn.jsdelivr.net/gh/mickmick1986/mick-fund-investment@main/deploy/live_estimates.json',
  'https://fastly.jsdelivr.net/gh/mickmick1986/mick-fund-investment@main/deploy/live_estimates.json',
  'https://api.github.com/repos/mickmick1986/mick-fund-investment/contents/deploy/live_estimates.json?ref=main',
  'https://raw.githubusercontent.com/mickmick1986/mick-fund-investment/main/deploy/live_estimates.json',
  'live_estimates.json'
];
var LIVE_SNAPSHOT_TIMEOUT_MS = 12000;
var LIVE_SNAPSHOT_MAX_AGE_MINUTES = 20;
var liveSnapshotPromise = null;
var liveSnapshotDiagnostics = [];

function decodeGitHubContent(payload) {{
  if (!payload || payload.encoding !== 'base64' || !payload.content) return payload;
  var binary = atob(String(payload.content).replace(/\s/g, ''));
  var bytes = new Uint8Array(binary.length);
  for (var i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
  return JSON.parse(new TextDecoder('utf-8').decode(bytes));
}}

function isFreshLiveSnapshot(snapshot) {{
  if (!snapshot || snapshot.trade_date !== getShanghaiClock().date || !snapshot.estimates) return false;
  var mode = getRefreshMode();
  if (mode !== 'slow' && mode !== 'fast') return true;
  var matched = /^\d{{4}}-\d{{2}}-\d{{2}}\s+(\d{{2}}):(\d{{2}})/.exec(snapshot.updated_at || '');
  if (!matched) return false;
  var snapshotMinutes = Number(matched[1]) * 60 + Number(matched[2]);
  var age = getShanghaiClock().minutes - snapshotMinutes;
  return age >= 0 && age <= LIVE_SNAPSHOT_MAX_AGE_MINUTES;
}}

function fetchSnapshot(url, cacheBuster) {{
  var separator = url.indexOf('?') >= 0 ? '&' : '?';
  var controller = new AbortController();
  var timeout = setTimeout(function() {{ controller.abort(); }}, LIVE_SNAPSHOT_TIMEOUT_MS);
  return fetch(url + separator + cacheBuster, {{ cache: 'no-store', signal: controller.signal }})
    .then(function(response) {{
      if (!response.ok) throw new Error('HTTP ' + response.status);
      return response.json();
    }})
    .then(function(payload) {{
      var snapshot = decodeGitHubContent(payload);
      if (!isFreshLiveSnapshot(snapshot)) throw new Error('snapshot stale');
      return snapshot;
    }})
...
function loadLiveSnapshot() {{
  ...
  function loadAt(index) {{
    if (index >= LIVE_SNAPSHOT_URLS.length) return Promise.resolve(null);
    var source = LIVE_SNAPSHOT_URLS[index];
    return fetchSnapshot(source, cacheBuster)
      .then(function(snapshot) {{
        liveSnapshotDiagnostics.push({{ source: source, outcome: 'success', updated_at: snapshot.updated_at }});
        return snapshot;
      }})
      .catch(function(error) {{
        liveSnapshotDiagnostics.push({{ source: source, outcome: 'failed', reason: String(error && error.message || error) }});
        return loadAt(index + 1);
      }});
  }}
  liveSnapshotPromise = loadAt(0);
...
function getLiveAdvice(drawdownPct, rsi, valSignal, trend20dPct, regimeParams) {{
  ...
  // 买入侧固定遵守三指标边界：RSI>70即不追高；止盈阈值不参与该判断。
  var overbought = isFinite(rsi) && rsi > 70;
...
async function updateAllFunds() {{
  ...
  if (liveTimeEl && snapshot && snapshot.updated_at) {{
    ...
    liveTimeEl.textContent = '实时快照不可用，保留确认数据';
    liveTimeEl.style.color = '#ffd6d6';
  }}
}}
