...
// 盘中优先用 <script> 直连新浪行情。脚本标签不受 fetch/CORS 限制，页面无需等待后台定时任务。
// 同源 JSON 仅作为首次加载、午间和数据源暂时不可用时的回退。
var LIVE_SNAPSHOT_URL = 'live_estimates.json';
var SINA_QUOTE_URL = 'https://hq.sinajs.cn/list=';
var liveSnapshotPromise = null;

function loadSinaQuotes(codes) {{
  return new Promise(function(resolve) {{
    if (!codes || !codes.length) {{ resolve({{}}); return; }}
    ...
    var finish = function() {{
      ...
      codes.forEach(function(code) {{
        ...
        // Sina: 名称, 时间, 估算净值, 昨日确认净值, ..., 涨跌额, 涨跌幅, 日期
        if (fields.length < 8 || fields[7] !== today) return;
        var estimateNav = parseFloat(fields[2]);
        var confirmedNav = parseFloat(fields[3]);
        if (!isFinite(estimateNav) || !isFinite(confirmedNav) || estimateNav <= 0 || confirmedNav <= 0) return;
        results[code] = {{
          gszzl: roundTo((estimateNav - confirmedNav) / confirmedNav * 100, 4),
          gztime: fields[7] + ' ' + fields[1]
        }};
      }});
      script.remove();
      resolve(results);
    ...
    script.async = true;
    script.src = SINA_QUOTE_URL + codes.map(function(code) {{ return 'fu_' + code; }}).join(',') + '&_=' + Date.now();
...
function loadSinaSse() {{
  return new Promise(function(resolve) {{
    ...
    var finish = function() {{
      if (settled) return;
      settled = true;
      var raw = window.hq_str_s_sh000001;
      var fields = raw && typeof raw === 'string' ? raw.split(',') : [];
      var close = parseFloat(fields[1]);
      var change = parseFloat(fields[2]);
      var changePct = parseFloat(fields[3]);
      script.remove();
      resolve(isFinite(close) && isFinite(changePct) ? {{ close: close, change: change, change_pct: changePct }} : null);
    }};
    script.async = true;
    script.src = SINA_QUOTE_URL + 's_sh000001&_=' + Date.now();
    script.onload = finish;
    script.onerror = finish;
    document.head.appendChild(script);
    setTimeout(finish, 8000);
...
// 盘中优先直接从新浪脚本读取；同源 JSON 只提供基础输入与不可用时的回退。
async function updateAllFunds() {{
  ...
  if (mode === 'slow' || mode === 'fast') {{
    directEstimates = await loadSinaQuotes(funds.map(function(fd) {{ return fd.code; }}));
    directSse = await loadSinaSse();
  }}
  var hasDirectQuotes = Object.keys(directEstimates).length > 0;
  var estimates = hasDirectQuotes ? directEstimates : (snapshot && snapshot.estimates ? snapshot.estimates : {{}});
  ...
  updateSseIndex(directSse ? {{ sse_index: directSse }} : snapshot);
  updateSummary();
  ...
  if (liveTimeEl) {{
    if (hasDirectQuotes) {{
      var now = new Date();
      var timeText = new Intl.DateTimeFormat('en-CA', {{ timeZone: 'Asia/Shanghai', hour: '2-digit', minute: '2-digit', second: '2-digit', hourCycle: 'h23' }}).format(now);
      liveTimeEl.textContent = '实时直连: ' + getShanghaiClock().date + ' ' + timeText + '（新浪）';
      liveTimeEl.style.color = '#dbeeff';
    }} else if (snapshot && snapshot.updated_at) {{
      liveTimeEl.textContent = '快照回退: ' + snapshot.updated_at;
      liveTimeEl.style.color = '#dbeeff';
    }}
  }}
}}
